...
export class InvestmentPropertyNoteGenerator {
  ...
  static generateWithRowTracking(
    ...
    const normalizeTrialBalance = (entries: TrialBalanceEntry[], predicate: (entry: TrialBalanceEntry) => boolean): InvestmentPropertyAccount[] => {
      return entries
        .filter(predicate)
        .map(entry => {
    ...
    const depreciationFallback = normalizeTrialBalance(trialBalanceData, entry => {
      ...
      return Number.isFinite(base) && base >= 1700 && base <= 1759;
    });
