// ============================================================================
// OTHER CURRENT LIABILITIES NOTE GENERATOR
// ============================================================================
...
/**
 * Generates other current liabilities note with row tracking
 */
export class OtherCurrentLiabilitiesNoteGenerator {
  
  /**
   * Generate other current liabilities note with row tracking
   */
  static generateWithRowTracking(
    ...
    const selRows = selection?.byCategory?.other_current_liabilities ?? [];
    if (selRows.length > 0) {
      let suppressed = 0;
      ...
      if (detailRows.length === 0 || totalsAreZero) {
        console.log('[SelectionFirst] Other current liabilities: skipping note - totals zero after filtering or no detail rows.');
        return tracker;
      }

      console.log(`[SelectionFirst] Other current liabilities: using selection-first details (${detailRows.length} accounts after suppressing ${suppressed}). Total current=${totalCurrent}, previous=${totalPrevious}`);

      // 1. Note Header Row
      notes.push([noteNumber.toString(), 'หนี้สินหมุนเวียนอื่น', '', '', '', '', '', '', 'หน่วย:บาท']);
    ...
    console.log('[SelectionFirst] Other current liabilities: no selection found, skipping note.');
    return tracker;
  }
}
